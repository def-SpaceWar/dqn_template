...
const agent1 = new DQNAgent(makeModel(12, 4));
const agent2 = new DQNAgent(makeModel(12, 4));
...
const getReward = (agent: DQNAgent, main: Player, other: Player) => {
    let reward = 0;
    if (Math.abs(main.pos[0] - width) > 200) reward -= 1;
    if (Math.abs(main.pos[0] - width) < 200) reward += 1;
    reward += 100 / distanceSquared(main, other);
    if (distanceSquared(main, other) < 10_000 && main.attackCooldown <= 0 && agent.action == 3) {
        reward += 100;
    }
    return reward;
};
